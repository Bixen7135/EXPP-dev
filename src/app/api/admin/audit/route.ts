...
// GET — admin (full) or teacher (own events) audit log
...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  const filters = {
    ...
    userId: session.role === "ADMIN" ? (sp.get("userId") ?? undefined) : undefined,
  ...
  const result = await getAuditLog(session.role as "ADMIN" | "TEACHER", session.id, filters);
